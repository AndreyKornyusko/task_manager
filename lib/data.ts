...
const initialData: DataStore = {
  tasks: [
    ...
    },
    {
      ...
      description: 'Create wireframes and mockups for the new dashboard features',
      priority: 'high',
      dueDate: '2024-12-18',
      status: 'todo',
      completed: false,
    ...
    },
    {
      ...
      description: 'Set up user authentication with JWT tokens and session management',
      priority: 'high',
      dueDate: '2024-12-22',
      status: 'in-progress',
      completed: false,
    ...
    },
    {
      ...
      description: 'Add comprehensive unit tests for all utility functions and components',
      priority: 'medium',
      dueDate: '2024-12-19',
      status: 'todo',
      completed: false,
    ...
    },
    {
      ...
      description: 'Address mobile responsiveness problems on smaller screens',
      priority: 'medium',
      dueDate: '2024-12-23',
      status: 'todo',
      completed: false,
    ...
    },
    {
      ...
      description: 'Clean up and refactor old code to follow modern best practices',
      priority: 'low',
      dueDate: '2024-12-26',
      status: 'todo',
      completed: false,
    ...
    },
    {
      ...
      description: 'Add search feature to filter and find tasks quickly',
      priority: 'medium',
      dueDate: '2024-12-28',
      status: 'todo',
      completed: false,
    ...
    },
    {
      ...
      description: 'Implement real-time notifications for task updates and deadlines',
      priority: 'high',
      dueDate: '2024-12-29',
      status: 'in-progress',
      completed: false,
...
function initializeStore() {
  if (dataStore.tasks.length === 0 && dataStore.subtasks.length === 0) {
    dataStore = {
      tasks: JSON.parse(JSON.stringify(initialData.tasks)),
      subtasks: JSON.parse(JSON.stringify(initialData.subtasks)),
    }
  }
}
...
export const dataStoreAPI = {
  ...
  getAllTasks: (): Task[] => {
    initializeStore()
    return dataStore.tasks.map((task) => ({
      ...task,
      subtasks: dataStore.subtasks.filter((s) => s.taskId === task.id),
    }))
  },
  ...
  deleteTask: (id: string): boolean => {
    ...
    dataStore.subtasks = dataStore.subtasks.filter((s) => s.taskId !== id)
    return true
  },
  ...
  getSubtasksByTaskId: (taskId: string): Subtask[] => {
    initializeStore()
    return dataStore.subtasks.filter((s) => s.taskId === taskId)
  },

  getSubtaskById: (id: string): Subtask | undefined => {
    initializeStore()
    return dataStore.subtasks.find((s) => s.id === id)
  },
  ...
  createSubtask: (subtask: Omit<Subtask, 'id'>): Subtask => {
    ...
    dataStore.subtasks.push(newSubtask)
    return newSubtask
  },
  ...
  deleteSubtask: (id: string): boolean => {
    ...
    dataStore.subtasks.splice(index, 1)
    return true
  },
}
